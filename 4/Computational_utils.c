...
void quadratic_interpolation(Vector *x, Vector *y, double upper_bound, double lower_bound, size_t samples){
	...
	coefs.rows = x->size;
	coefs.cols = x->size + 1;
	coefs.data = malloc(sizeof(double)*coefs.rows*coefs.cols);
	for(int i=0; i<coefs.rows*coefs.cols; i++){
		int r = i % coefs.cols;
		int p = i / coefs.cols;
		if(r == 0) coefs.data[i] = 1;
		else if(r == 1) coefs.data[i] = x->data[p];
	       	else if(r == 2) coefs.data[i] = x->data[p] * x->data[p];
		else coefs.data[i] = y->data[p];
	}
	
	Vector solutions = cramers_rule(&coefs);
	inline double quad(Vector *vec, double x){
		return vec->data[0] + vec->data[1] * x + x *x * vec->data[2];
	}
	double delta_x = (upper_bound - lower_bound) / (double) samples;
	for(int i=0; i<samples; i++){
		double x = lower_bound + i * delta_x;
		printf("x[%d] = %lf; f(x) = %lf\n", i, x, quad(&solutions,x));
	}
}
...
Vector fit(Vector *x, Vector *y){
	if(x->size != y->size) {
		printf("Computational_utils::fit: Vectors x and y must of the same size.\n");
		exit(1);
	}

	Vector coefs;
	double arr[] = {0, 0};
	init_vec(&coefs, 2, arr);

	double a1_t = 0;
	double a1_b = 0;
	double avg_x = avg_vec(x);
	double avg_y = avg_vec(y);
	for(int i=0; i<x->size; i++) a1_t += (x->data[i] - avg_x)*(y->data[i] - avg_y);
	for(int i=0; i<x->size; i++) a1_b += (x->data[i] - avg_x)*( x->data[i] - avg_x);
	coefs.data[1] =  a1_t / a1_b;
	coefs.data[0] = avg_y - coefs.data[1] * avg_x;
	return coefs;
}
